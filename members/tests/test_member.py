...
def test_retrieve_memebrs_list(self): #Test retrieving a list of members
...
def test_create_persona_successful(self): #Testing create a new person
  payload = {'first_name':'Carolina',  'last_name':'Mendoza', 
          'last_name_m':'Correa', 'genero':1,
          'birth_date':'1989-04-23', 'civil_status':1, 'age':33,
          'state':3, 'residence':'utopia', 'area':1, 'jobrole':1, 'modalidad':1, 'tipo_trabajo':1}
  self.client.post(INGREDIENTS_URL, payload)

  exists =  Persona.objects.filter(
    user = self.user,
    first_name = payload['first_name'],).exists() #chechking if person exists. exists() function will return boolean true or false value
